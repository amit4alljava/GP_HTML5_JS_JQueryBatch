...
var taskOperations= {
    ...
    getTaskByName: function (taskName) {
        ...
        var index = 0;
        for (var i = 0; i < this.taskList.length; i++) {
            console.log("List Name "+this.taskList.taskName+ " task Name "+taskName+" Index is  "+this.taskList[i].taskName.indexOf(taskName));
            if (this.taskList[i].taskName.indexOf(taskName)>=0) {

                searchTaskList[index] = this.taskList[i];
                index++;
            }
        }
